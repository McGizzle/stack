...
data Package =
  Package {packageName              :: !PackageName                    -- ^ Name of the package.
          ...
          ,packageFiles             :: !(PackageDescription)               -- ^ Get all files of the package.
...
data TPackage =
  TPackage {tpackageName             :: !PackageName                    -- ^ Name of the package.
          ,tpackageVersion           :: !Version                     -- ^ Version of the package
          ,tpackageLicense           :: !License                     -- ^ The license the package was released under.
          ,tpackageFiles             :: !PackageFiles               -- ^ Get all files of the package.
          ,tpackageDeps              :: !(Map PackageName VersionRange) -- ^ Packages that the package depends on.
          ,tpackageTools             :: !(Map ExeName VersionRange)    -- ^ A build tool name.
          ,tpackageAllDeps           :: !(Set PackageName)           -- ^ Original dependencies (not sieved).
          ,tpackageGhcOptions        :: ![Text]                   -- ^ Ghc options used on package.
          ,tpackageFlags             :: !(Map FlagName Bool)           -- ^ Flags used on package.
          ,tpackageDefaultFlags      :: !(Map FlagName Bool)    -- ^ Defaults for unspecified flags.
          ,tpackageLibraries         :: !PackageLibraries          -- ^ does the package have a buildable library stanza?
          ,tpackageTests             :: !(Map Text TestSuiteInterface) -- ^ names and interfaces of test suites
          ,tpackageBenchmarks        :: !(Set Text)               -- ^ names of benchmarks
          ,tpackageExes              :: !(Set Text)                     -- ^ names of executables
          ,tpackageOpts              :: !PackageOpts                 -- ^ Args to pass to GHC.
          ,tpackageHasExposedModules :: !Bool              -- ^ Does the package have exposed modules?
          ,tpackageBuildType         :: !(Maybe BuildType)         -- ^ Package build-type.
          ,tpackageSetupDeps         :: !(Maybe (Map PackageName VersionRange))
                                                          -- ^ If present: custom-setup dependencies
          }
 deriving (Show,Typeable,Generic)

instance Binary TPackage
...
data PackageOpts = PackageOpts (Map NamedComponent (Map ModuleName (Path Abs File))
                   ,Map NamedComponent (Set DotCabalPath)
                   ,Map NamedComponent BuildInfoOpts)
 deriving(Typeable, Generic)
instance Show PackageOpts where
        show _ = "<PackageOpts>"
instance Binary PackageOpts
...
data PackageFiles = PackageFiles (Map NamedComponent (Map ModuleName (Path Abs File))
                    ,Map NamedComponent (Set DotCabalPath)
                    ,Set (Path Abs File)
                    ,[PackageWarning])
  deriving(Typeable, Generic)
instance Show PackageFiles where
    show _ = "<PackageFiles>"
instance Binary PackageFiles
-- | Files that the package depends on, relative to package directory.
-- Argument is the location of the .cabal file
...
instance Binary InstallLocation
