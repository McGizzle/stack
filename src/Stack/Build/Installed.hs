...
getInstalled opts sourceMap = do
    ...
    (installedLibs1, _extraInstalled) <-
        foldM
            (\lhs' pkgdb -> loadDatabase' (Just (ExtraGlobal, pkgdb)) (fst lhs'))
            (installedLibs0, globalDumpPkgs)
            extraDBPaths
        ...
        exeToSM loc (PackageIdentifier name version) =
          ...
          where
            m =
                Map.singleton
                    name
                    (loc, Executable $ PackageIdentifier name version)
    ...
    return (installedMap, globalDumpPkgs, snapshotDumpPkgs, localDumpPkgs)
...
loadDatabase opts mcache sourceMap mdb lhs0 = do
    ...
    (lhs1', dps) <-
        ghcPkgDump wc (fmap snd (maybeToList mdb)) $ conduitDumpPackage .| sink
  ...
  where
    conduitProfilingCache =
        case mcache of
            Just cache
                | getInstalledProfiling opts -> addProfiling cache
    ...
    conduitHaddockCache =
        case mcache of
            Just cache
                | getInstalledHaddock opts -> addHaddock cache
    ...
    conduitSymbolsCache =
        case mcache of
            Just cache
                | getInstalledSymbols opts -> addSymbols cache
    ...
    sinkDP =
        ...
        CL.consume
    sink = getZipSink $ (,) <$> ZipSink sinkDP <*> ZipSink CL.consume
...
processLoadResult mdb _ (reason, lh) = do
    logDebug $
        T.concat $
        ["Ignoring package ", packageNameText (fst (lhPair lh))] ++
        ...
        , case reason of
              ...
              WrongLocation mloc loc ->
                  " wrong location: " <> T.pack (show (mloc, loc))
...
isAllowed opts mcache sourceMap mloc dp
    ...
    | getInstalledProfiling opts && isJust mcache && not (dpProfiling dp) =
        NeedsProfiling
    -- Check that it has haddocks if necessary
    | getInstalledHaddock opts && isJust mcache && not (dpHaddock dp) =
        NeedsHaddock
    -- Check that it has haddocks if necessary
    | getInstalledSymbols opts && isJust mcache && not (dpSymbols dp) =
        NeedsSymbols
    | otherwise =
        case Map.lookup name sourceMap of
            Nothing ->
                case mloc
                    ...
                    Just _           -> UnknownPkg
            Just pii
                | not (checkLocation (piiLocation pii)) ->
                    WrongLocation mloc (piiLocation pii)
                | version /= piiVersion pii ->
                    WrongVersion version (piiVersion pii)
                | otherwise -> Allowed
  where
...
toLoadHelper mloc dp =
    ...
    , lhPair =
          (name, (toPackageLocation mloc, Library ident gid (dpLicense dp)))
    }
  where
    gid = dpGhcPkgId dp
