...
type ParentMap
     = MonoidMap PackageName ( First Version
...
type M
     = RWST -- TODO replace with more efficient WS stack on top of StackT
        Ctx W (Map PackageName (Either ConstructPlanException AddDepRes)) IO
   ...
   -- logInfo $ T.pack $ "\n\nConstructBuildPlan -> lpProject: "++ show (Map.keys (lpProject lp))
    ...
    if null errs
        then do
            ...
            prettyErrorNoIndent $
                pprintExceptions errs stackYaml parents (wanted ctx)
      ...
      where
        us =
            execState
                (mapM_ go keep)
                UnregisterState
                {usToUnregister = toUnregister, usKeep = [], usAnyAdded = False}
    go dp = do
        us <- get
        case go' (usToUnregister us) ident deps
            ...
            ----- DEBUG -----------------------------------------------------------------------------------------------------------
                    { usToUnregister =
                          Map.insert gid (ident, reason) (usToUnregister us)
                    , usAnyAdded = True
                    }
            -----------------------------------------------------------------------------------------------------------------------
      where
    ...
    go' toUnregister ident deps
      ...
      -- build is being done.
        | Just task <- Map.lookup name tasks =
            if initialBuildSteps &&
               taskIsTarget task && taskProvides task == ident
                then Nothing
      ...
      -- Check if a dependency is going to be unregistered
        | (dep, _):_ <- mapMaybe (`Map.lookup` toUnregister) deps =
            Just $
            "Dependency being unregistered: " <> packageIdentifierText dep
            ...
            Right (missing, present, _minLoc) -> do
                ...
                return $
                    Right
                        ...
                        , taskCachePkgSrc =
                              CacheSrcLocal (toFilePath (lpDir lp))
...
addDep ::
       Bool -- ^ is this being used by a dependency?
    -> PackageName
...
addDep treatAsDep' name = do
    ...
    case Map.lookup name m of
        Just res -> do
            planDebug $
                "addDep: Using cached result for " ++
                show name ++ ": " ++ show res
            return res
        Nothing -> do
            res <-
                if name `elem` callStack ctx
                    then do
                        planDebug $
                            ...
                            show name ++ ": " ++ show (callStack ctx)
                        return $
                            Left $
                            DependencyCycleDetected $ name : callStack ctx
                    else local
                             (\ctx' -> ctx' {callStack = name : callStack ctx'}) $ do
                             let mpackageInfo =
                                     Map.lookup name $ combinedMap ctx
                             planDebug $
                                 "addDep: Package info for " ++
                                 show name ++ ": " ++ show mpackageInfo
                             case mpackageInfo
...
tellExecutablesUpstream ::
       PackageIdentifierRevision -> InstallLocation -> Map FlagName Bool -> M ()
            ...
            case lpTestBench lp of
                Nothing -> do
                    planDebug $
                        ...
                        show name ++ " so doing an all-in-one build."
                 ...
                 -> do
                    ...
                    case res of
                        Right deps -> do
                            planDebug $
                                "installPackage: For " ++
                                show name ++ ", successfully added package deps"
                            adr <-
                        ...
                        Left _
                         ...
                         -> do
                            planDebug $
                                "installPackage: Before trying cyclic plan, resetting lib result map to " ++
                                show s
                            put s
...
resolveDepsAndInstall isAllInOne treatAsDep ps package minstalled = do
    ...
    case res of
        ...
        Right deps ->
            liftM Right $
            installPackageGivenDeps isAllInOne ps package minstalled deps
...
installPackageGivenDeps ::
    ...
    -> Package
    -> Maybe Installed
    -> (Set PackageIdentifier, Map PackageIdentifier GhcPkgId, InstallLocation)
    -> M AddDepRes
installPackageGivenDeps isAllInOne ps package minstalled (missing, present, minLoc) = do
    let name = packageName package
    ctx <- ask
    mRightVersionInstalled <-
        case (minstalled, Set.null missing) of
            (Just installed, True) -> do
                shouldInstall <-
                    checkDirtiness ps installed package present (wanted ctx)
                return $
                    if shouldInstall
                        then Nothing
                        else Just installed
            (Just _, False) -> do
                let t =
                        T.intercalate ", " $
                        map
                            (T.pack . packageNameString . packageIdentifierName)
                            (Set.toList missing)
                tell
                    mempty
                    { wDirty =
                          Map.singleton name $
                          "missing dependencies: " <> addEllipsis t
                    }
                return Nothing
            (Nothing, _) -> return Nothing
    return $
        case mRightVersionInstalled of
                ...
                ADRToInstall
                    ...
                    , taskType =
                          case ps of
                              ...
                              PSIndex loc _ _ pkgLoc ->
                                  TTIndex package (loc <> minLoc) pkgLoc
                    , taskAllInOne = isAllInOne
...
addPackageDeps ::
       Bool -- ^ is this being used by a dependency?
    -> Package
    -> M (Either ConstructPlanException ( Set PackageIdentifier
...
addPackageDeps treatAsDep package = do
    ...
    deps <-
        forM (Map.toList deps') $ \(depname, (range, depType)) -> do
            ...
            case eres of
                Left e -> do
                    ...
                    let bd =
                            case e of
                                UnknownPackage name ->
                                    assert (name == depname) NotInBuildPlan
                                _ ->
                ...
                Right adr
                    | depType == AsLibrary && not (adrHasLibrary adr) ->
                        return $ Left (depname, (range, Nothing, HasNoLibrary))
                Right adr -> do
                    addParent depname range Nothing
                    inRange <-
                        if adrVersion adr `withinRange` range
                            then return True
                            else do
                                let warn_ reason =
                                ...
                                allowNewer <-
                                    view $ configL . to configAllowNewer
                                if allowNewer
                                    then do
                                        ...
                                        if x && y
                                            then do
                                                warn_
                                                    " (trusting snapshot over Hackage revisions)"
                                                return True
                                            else return False
                    if inRange
                        then case adr of
                                 ...
                                 ADRFound loc (Executable _) ->
                                     return $ Right (Set.empty, Map.empty, loc)
    ...
    case partitionEithers deps
        ...
        (errs, _) ->
            return $ Left $ DependencyPlanFailures package (Map.fromList errs)
  where
    adrVersion (ADRToInstall task) =
        packageIdentifierVersion $ taskProvides task
...
checkDirtiness ps installed package present wanted = do
    ...
    let mreason =
            case moldOpts of
                ...
                Just oldOpts
                    | Just reason <-
                         describeConfigDiff config oldOpts wantConfigCache ->
                        Just reason
...
describeConfigDiff config old new
    | configCachePkgSrc old /= configCachePkgSrc new =
        Just $
        "switching from " <> pkgSrcName (configCachePkgSrc old) <> " to " <>
    ...
    | oldOpts /= newOpts =
        Just $
        T.pack $
        concat ["flags changed from ", show oldOpts, " to ", show newOpts]
    | otherwise = Nothing
  where
    stripGhcOptions = go
      where
        ...
        checkKeepers wc x xs =
            case filter isKeeper $ T.words x of
                [] -> xs
                keepers ->
                    T.pack (compilerOptionsCabalFlag wc) :
                    T.unwords keepers : xs
    ...
    userOpts =
        filter (not . isStackOpt) .
        (if configRebuildGhcOptions config
             then id
             else stripGhcOptions) .
        map T.pack . (\(ConfigureOpts x y) -> x ++ y) . configCacheOpts
...
packageDepsWithTools p = do
    ...
    let toEither name mp =
        ...
        (warnings0, toolDeps) =
            partitionEithers $
            map
                (\dep -> toEither dep (toolToPackages ctx dep))
                (Map.keys (packageTools p))
    -- Check whether the tool is on the PATH before warning about it.
    warnings <-
        fmap catMaybes $
        forM warnings0 $ \warning@(ToolWarning (ExeName toolName) _ _) -> do
...
data ToolWarning =
    ToolWarning ExeName
                PackageName
                (Maybe (PackageName, PackageName, [PackageName]))
    deriving (Show)

toolWarningText :: ToolWarning -> Text
toolWarningText (ToolWarning (ExeName toolName) pkgName Nothing) =
    "No packages found in snapshot which provide a " <> T.pack (show toolName) <>
...
pprintExceptions exceptions stackYaml parentMap wanted =
    ...
    , line <> line
    , mconcat
          (intersperse (line <> line) (mapMaybe pprintException exceptions'))
    , line <> line
  ...
  where
    ...
    getExtras (DependencyPlanFailures _ m) = Map.unions $ map go $ Map.toList m
    ...
    onlyHasDependencyMismatches = all go exceptions'
      where
        ...
        isMismatch _                                 = False
    pprintException (DependencyCycleDetected pNames) =
        Just $
    ...
    pprintException (DependencyPlanFailures pkg pDeps) =
        case mapMaybe pprintDep (Map.toList pDeps) of
            ...
            depErrors ->
                ...
                pkgIdent <> pprintFlags (packageFlags pkg) <> ":" <> line <>
                ...
                case getShortestDepsPath parentMap wanted (packageName pkg) of
                    ...
                    Just [] ->
                        line <> flow "needed since" <+>
                        pkgName <+> flow "is a build target."
                    Just (target:path) ->
                        line <> flow "needed due to" <+>
                        encloseSep "" "" " -> " pathElems
                        where pathElems =
                                  [styleTarget . display $ target] ++
                                  map display path ++ [pkgIdent]
                where pkgName = styleCurrent . display $ packageName pkg
    ...
    pprintException (UnknownPackage name)
        ...
        | name `HashSet.member` wiredInPackages =
            ...
            (styleCurrent . display $ name)
        | otherwise =
            Just $ flow "Unknown package:" <+> (styleCurrent . display $ name)
    pprintFlags flags
        | Map.null flags = ""
    ...
    pprintDep (name, (range, mlatestApplicable, badDep)) =
        case badDep of
            NotInBuildPlan ->
                Just $
            ...
            DependencyMismatch version ->
                ...
                align
                    (flow "from stack configuration does not match" <+>
                     goodRange <> latestApplicable (Just version))
      ...
      where
        ...
        latestApplicable mversion =
            case mlatestApplicable of
                Nothing -> ""
                Just la
                    | mlatestApplicable == mversion ->
                        softline <>
                        flow "(latest matching version is specified)"
                    | otherwise ->
                        softline <> flow "(latest matching version is" <+>
                        styleGood (display la) <> ")"
...
-- package.
getShortestDepsPath ::
       ParentMap -> Set PackageName -> PackageName -> Maybe [PackageIdentifier]
getShortestDepsPath (MonoidMap parentsMap) wanted name =
  ...
  where
    ...
    findShortest _ paths
        | M.null paths = []
    findShortest fuel paths =
        case targets of
            [] ->
                findShortest (fuel - 1) $
                M.fromListWith chooseBest $ concatMap extendPath recurses
            _ ->
                let (DepsPath _ _ path) = minimum (map snd targets)
                in path
      where
        (targets, recurses) =
            partition (\(n, _) -> n `Set.member` wanted) (M.toList paths)
    ...
    extendPath (n, dp) =
        case M.lookup n parentsMap of
            ...
            Just (_, parents) ->
                map
                    (\(pkgId, _) ->
                         (packageIdentifierName pkgId, extendDepsPath pkgId dp))
                    parents
...
extendDepsPath ident dp =
    DepsPath
    { dpLength = dpLength dp + 1
    , dpNameLength =
          dpNameLength dp +
          T.length (packageNameText (packageIdentifierName ident))
    , dpPath = [ident]
    }
...
instance (Ord k, Monoid a) => Monoid (MonoidMap k a) where
    mappend (MonoidMap mp1) (MonoidMap mp2) =
        MonoidMap (M.unionWith mappend mp1 mp2)
