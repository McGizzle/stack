...
-- | Perform a build
module Distributed.Build.Execute
    ( printPlan
    , preFetch
    , executePlan
    -- * Running Setup.hs
...
import           Control.Concurrent               hiding (newMVar, withMVar,
                                                   yield)
import           Control.Concurrent.MVar          hiding (newMVar, withMVar)
...
import           System.IO                        (hPutStr, print, readFile,
                                                   stderr, stdout)
            ...
            (xs, action:ys) -> do
               ...
               where
                       ...
                       mask' as' remaining = mask $ \restore -> do
                        ...
                        eres <- runActionDist (actionInfo action) context
                        atomically $ do
...
toActions :: HasEnvConfig env
          ...
          -> [Action]
toActions installedMap mtestLock runInBase ee (mbuild, mfinal) envConfig = abuild ++ afinal
  --- DEBUG ----------------------------------------------------------------------------------------
        ...
        case mbuild of
          ...
          ----------------------------------------------------------------
                [ Action
                    ...
                    , actionInfo = BuildInfo ee task installedMap False envConfig
