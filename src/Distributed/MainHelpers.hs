  ...
  where
    ...
    addCommands = do
      unless isInterpreter (do
        addBuildCommand' "distributed"
                         "Loads Stack to be ready for distribution"
                         buildCmd
                         (buildOptsParser Distributed)
...
buildCmd opts go = do
  ...
  where
    inner setLocalFiles = withBuildConfigAndLock go' $ \lk ->
          Stack.Build.build setLocalFiles lk opts dist
    ...
    go' = case boptsCLICommand opts of
               ...
               _     -> go
    dist = case boptsCLICommand opts of
             Distributed -> True
             _ -> False
